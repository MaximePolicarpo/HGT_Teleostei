...
current_tree <- 
  read.tree("N5.HOG0001647.prot.aln.treefile")
...
#Define wich gene are part of candidate HGT events, and with which ks filter. First, to reduce computation, retain Hypomesus and Clupea species from the HGT table


HGT_candidate_df <- HGT_candidate_df %>% filter(species_1 == "Hypomesus_transpacificus" | species_2 == "Hypomesus_transpacificus")
HGT_candidate_df <- HGT_candidate_df %>% filter(species_1 == "Clupea_harengus" | species_2 == "Clupea_harengus")
...
#Add the alignment
prot_alignment = "N5.HOG0001647.prot.aln"
prot_alignment_trim = "N5.HOG0001647.prot.trimmed.aln"
...
current_tree <- 
  read.tree("N5.HOG0001647/HGT_clade.cds.aln.treefile")
...
DNA_tree <- 
  ...
  xlim(0, 0.1) +
  geom_treescale(x=0.01, y=8, width=0.01, color='black') 
...
MRCA_outgroup <- findMRCA(current_tree, tips=outgroup_id, type="node")
...
p2 <- msaplot(p0, prot_alignment, offset=1, width=3)
p3 <- msaplot(p0, prot_alignment_trim, offset=1, width=3)
...
#### Species Ks distribution   ---------------------------------


current_ks_df <- 
  fread("Hypomesus_transpacificus.cds.stats",
        header=FALSE,
        sep=",")

curr_stats_df <-
  fread("Hypomesus_transpacificus.csv",
        header=TRUE,
        sep=",")
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 == "Hypomesus_transpacificus_rna_XM_047032478_1") 


pdf("ks_distribution.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Hypomesus_transpacificus vs Clupea_harengus   ---------------------------------


current_ks_df <- 
  fread("Clupea_harengus.cds.stats",
        header=FALSE,
        sep=",")

curr_stats_df <-
  fread("Clupea_harengus.csv",
        header=TRUE,
        sep=",")
...
species2 <- "Hypomesus_transpacificus"
...
#current_ks_df %>%
#  filter(OGG == "N5.HOG0004401") %>% pull(seq1) %>% unique()
  
#putative_TE_genes[putative_TE_genes == "Clupea_harengus_rna_XM_031578252_1"]
...
write.table(current_ks_df %>% dplyr::select(OGG, seq1, seq2),
            "All_pairwise_genes.Charengus.Htranspacificus.csv",
            sep=",",
            col.names=FALSE,
            row.names=FALSE,
            quote=FALSE)
...
current_ks_HGT <-
  current_ks_df %>%
  filter(OGG == "N5.HOG0001647")

length(current_ks_df %>% filter(ks < 0.376) %>% pull(OGG) %>% unique())


pdf("ks_distribution.Hypomesus_transpacificus.clupea.pdf",width = 8.34,  height = 4.61)
...
my_seq1 <- c("Hypomesus_transpacificus_rna_XM_047032478_1")
my_seq2 <- c("Clupea_harengus_rna_XM_031563501_2",
             "Clupea_harengus_rna_XM_031564003_2",
             "Clupea_harengus_rna_XM_031564015_2",
             "Clupea_harengus_rna_XM_031564001_2",
             "Clupea_harengus_rna_XM_031563896_2",
             "Clupea_harengus_rna_XM_031581825_2",
             "Clupea_harengus_rna_XM_031581789_2",
             "Clupea_harengus_rna_XM_031581779_2")


HOG_stats_df_species <- 
  HOG_stats_df_species %>%
  mutate(group_hgt = if_else(
    (seq1 %in% my_seq1 & seq2 %in% my_seq2) | (seq1 %in% my_seq2 & seq2 %in% my_seq1),
    "HGT",
    "no_HGT"
  ))

HOG_stats_df_species %>% filter(group_hgt == "HGT")
...
pair_to_fix <- 
  c("Hypomesus_transpacificus,Osmerus_eperlanus")

for(species_pair in pair_to_fix){
...
for(species_pair in pair_to_fix_onlyscaff){
  ...
  my_links.clade1_new <- 
    my_links.clade1 %>% 
    filter(! (species1 == my_sp1 & species2 == my_sp2 & seq_id == "NC_085005_1"))
...
p.clade1 <-  
  ...
  flip_seqs("NC_085029_1") %>%
  flip_seqs("NC_085005_1") %>%
  flip_seqs("NC_063195_1") %>%
  flip_seqs("OX465232_1") %>%
  flip_seqs("NC_085874_1")
...
all_OGG_gray_vec["N5_HOG0001647"] <- "red"
...
all_OGG_gray_vec <- setNames(rep("gray", length(all_OGG)), all_OGG)
all_OGG_gray_vec["N5_HOG0001647"] <- "red"
all_OGG_gray_vec["Crack-7_SSa_2p:ClassI:LINE:Jockey:Crack"] <- "#FFC107"
...
#### Region alignment -- Hypomesus vs Osmerus  ---------------------------------



library(dotplot)

##BETWEEN HYPOMESUS AND OSMERUS


osmerus_region <- seqinr::read.fasta("osmerus_scaffold.rev.fa", seqtype = "DNA", as.string = TRUE)[[1]]
hypomesus_region <- seqinr::read.fasta("hypomesus_scaffold.fa", seqtype = "DNA", as.string = TRUE)[[1]]

p_dot_1 <- 
  dotPlotg(hypomesus_region, osmerus_region, wsize = 40, wstep = 1, nmatch = 40) +
  geom_rect(aes(xmin=6549, xmax=6656, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 1 AFP
  geom_rect(aes(xmin=6772, xmax=6862, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 2 AFP
  geom_rect(aes(xmin=7459, xmax=7538, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 3 AFP
  geom_rect(aes(xmin=7643, xmax=7768, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 4 AFP
  geom_rect(aes(xmin=7869, xmax=7977, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 5 AFP
  geom_rect(aes(xmin=8315, xmax=8500, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 6 AFP
  theme_classic() +
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18),
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("H. transpacificus (NC_061072.1:10548169-10591617)")+
  ylab("O. eperlanus (NC_085029.1:13548639-13575090)")


ggsave("dotplot_hypo_osm.pdf", p_dot_1, width=8.34, height=6.34, units="in", scale=2)



#### Region alignment -- Hypomesus vs Clupea  ---------------------------------


clupea_region_1 <- seqinr::read.fasta("N5.HOG0001647.Clupea_harengus.extended.1.fordotplot.fa", seqtype = "DNA", as.string = TRUE)[[1]]
hypomesus_region <- seqinr::read.fasta("N5.HOG0001647.Hypomesus_transpacificus.extended.fa", seqtype = "DNA", as.string = TRUE)[[1]]

p_dot_1 <- 
  dotPlotg(hypomesus_region, clupea_region_1, wsize = 40, wstep = 1, nmatch = 35) +
  geom_rect(aes(xmin=5000, xmax=5107, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 1 hypo AFP
  geom_rect(aes(xmin=5223, xmax=5313, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 2 hypo AFP
  geom_rect(aes(xmin=5910, xmax=5989, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 3 hypo AFP
  geom_rect(aes(xmin=6094, xmax=6219, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 4 hypo AFP
  geom_rect(aes(xmin=6320, xmax=6428, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 5 hypo AFP
  geom_rect(aes(xmin=6766, xmax=6951, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 6 hypo AFP
  
  geom_rect(aes(xmin=2010, xmax=2887, ymin=0, ymax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) + #CRACK7
  
  geom_rect(aes(ymin=5000, ymax=5153, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 1 clupea AFP1
  geom_rect(aes(ymin=5270, ymax=5360, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 2 clupea AFP1
  geom_rect(aes(ymin=5916, ymax=6043, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 3 clupea AFP1
  geom_rect(aes(ymin=6148, ymax=6273, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 4 clupea AFP1
  geom_rect(aes(ymin=6378, ymax=6486, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 5 clupea AFP1
  geom_rect(aes(ymin=6823, ymax=7004, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + #exon 6 clupea AFP1
 
  geom_rect(aes(ymin=2366, ymax=3203, xmin=0, xmax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) + #CRACK7
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("H. transpacificus (NC_061072.1:10549718-10561669)")+
  ylab("C. harengus (NC_045177.1:7930105-7942109)")

ggsave("dotplot_hypo_clupea1.pdf", p_dot_1, width=8.34, height=6.34, units="in", scale=2)
...
dotPlotg(hypomesus_region, clupea_region_1, wsize = 40, wstep = 1, nmatch = 35) +
  ...
  geom_rect(aes(xmin=1300, xmax=2000, ymin=0, ymax=Inf), color="black", fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=4700, xmax=5000, ymin=0, ymax=Inf), color="black", fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=6970, xmax=7500, ymin=0, ymax=Inf), color="black", fill=NA, linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=1700, ymax=2300, xmin=0, xmax=Inf), color="black", fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=4700, ymax=5000, xmin=0, xmax=Inf), color="black", fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=7050, ymax=7600, xmin=0, xmax=Inf), color="black", fill=NA, linewidth=0.2, alpha=0.4) + 
