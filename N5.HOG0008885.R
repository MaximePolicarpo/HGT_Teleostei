...
current_tree <- 
  read.tree("N5.HOG0008885.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0008885.prot.aln"
prot_alignment_trim = "N5.HOG0008885.prot.trimmed.aln"
...
current_tree <- 
  read.tree("N5.HOG0008885/HGT_clade.cds.aln.treefile")
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Hypomesus_transpacificus_rna_XM_047048755_1")) %>%
  filter(seq2 %in% c("Clupea_harengus_rna_XM_031562957_1")) 
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0008885.cds.stats",
        sep=",",
        header=TRUE)
...
HOG_stats_df_species$species_1 <- 
  gsub("Salvelinus_sp_IW2_2015", "Salvelinus_sp_IW2_2015", HOG_stats_df_species$species_1)
HOG_stats_df_species$species_2 <- 
  gsub("Salvelinus_sp_IW2_2015", "Salvelinus_sp_IW2_2015", HOG_stats_df_species$species_2)
...
my_seq1 <- c("Hypomesus_transpacificus_rna_XM_047048755_1")
my_seq2 <- c("Clupea_harengus_rna_XM_031562957_1")
...
pair_to_fix_onlyscaff <- 
  c(
    ...
    "Osmerus_eperlanus,Borostomias_antarcticus")
...
all_OGG_gray_vec["N5_HOG0008885"] <- "red"
...
all_OGG_gray_vec <- setNames(rep("gray", length(all_OGG)), all_OGG)
all_OGG_gray_vec["N5_HOG0008885"] <- "red"
all_OGG_gray_vec["RTEX-2_SSa_polLINE/RTE-X"] <- "#FFC107"
...
hypomesus_region_1 <- seqinr::read.fasta("N5.HOG0008885.Hypomesus_transpacificus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]
clupea_region_1 <- seqinr::read.fasta("N5.HOG0008885.Clupea_harengus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]

dotplot.p1 <- 
  dotPlotg(hypomesus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 36) +
  geom_rect(aes(xmin=1415, xmax=3082, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) +
  
  geom_rect(aes(ymin=5526, ymax=7556, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) +
  
  geom_rect(aes(xmin=3080, xmax=3228, ymin=0, ymax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) +
  
  geom_rect(aes(ymin=7596, ymax=7787, xmin=0, xmax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) +
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("H. transpacificus (NC_061062.1:1779381-1791080)")+
  ylab("C. harengus (NC_045176.1:498859-521756)")
...
dotPlotg(hypomesus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 36) +
  geom_rect(aes(xmin=4400, xmax=5000, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) +
  geom_rect(aes(xmin=6300, xmax=7100, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) +
  
  geom_rect(aes(ymin=7787, ymax=8887, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) +
  geom_rect(aes(ymin=17000, ymax=19000, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) 
