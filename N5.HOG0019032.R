...
current_tree <- 
  read.tree("N5.HOG0019032.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0019032.prot.aln"
prot_alignment_trim = "N5.HOG0019032.prot.trimmed.aln"
...
current_tree <- 
  read.tree("~/Horizontal_transfer_project/Candidates_analysis/N5.HOG0019032/HGT_clade.cds.aln.treefile")
...
DNA_tree <- 
  ...
  xlim(0, 2) +
  geom_treescale(x=0.02, y=25, width=0.2, color='black') 
...
ggsave("~/Horizontal_transfer_project/Candidates_analysis/N5.HOG0019032/DNA_subtree.pdf",
...
#### Draw  phylogenies with alignment -- Other Neoarius spp   ---------------------------------


current_tree <- 
  read.tree("Neoarius_spp.ChanosALT.HOG0019032.combined.prot.aln.treefile")
...
toadd <- as.data.frame(cbind("Neoarius_berneyi.GCA_027576835.1.JAODIF010000006.1-15939796-15974075", "Neoarius_graeffei", "Siluriformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

toadd <- as.data.frame(cbind("Neoarius_pectoralis.GCA_029814605.1.JAPTHB010000022.1-70174794-70186686", "Neoarius_graeffei", "Siluriformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)


toadd <- as.data.frame(cbind("Neoarius_leptaspis.GCA_025727735.1.JAODTU010000024.1-7650710-7664992", "Neoarius_graeffei", "Siluriformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

toadd <- as.data.frame(cbind("C.chanos.GCA_018691325.1.JAFFIB010000077.1-462554-471452", "Chanos_chanos", "Gonorynchiformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

toadd <- as.data.frame(cbind("C.chanos.GCA_018691325.1.JAFFIB010000115.1-1862497-1867291", "Chanos_chanos", "Gonorynchiformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

toadd <- as.data.frame(cbind("C.chanos.GCA_018691325.1.JAFFIB010000115.1-1837168-1845440", "Chanos_chanos", "Gonorynchiformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

toadd <- as.data.frame(cbind("C.chanos.GCA_018691325.1.JAFFIB010000115.1-1807781-1811600", "Chanos_chanos", "Gonorynchiformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

#Add the alignment
prot_alignment_trim= "Neoarius_spp.ChanosALT.HOG0019032.combined.prot.aln.trimmed"
...
pdf("N5.HOG0019032/Manually_annoated_genes.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Neoarius_graeffei vs Chanos_chanos   ---------------------------------


current_ks_df <- 
  fread("HGT_stats_Results_per_sp/Neoarius_graeffei.cds.stats",
        header=FALSE,
        sep=",")

curr_stats_df <-
  fread("KS_Stats_per_species/Neoarius_graeffei.csv",
        header=TRUE,
        sep=",")
...
write.table(current_ks_df %>% dplyr::select(OGG, seq1, seq2),
            "All_pairwise_genes.Chanos.Neoarius.csv",
            sep=",",
            col.names=FALSE,
            row.names=FALSE,
            quote=FALSE)
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Neoarius_graeffei_rna_XM_060939486_1",
                     "Neoarius_graeffei_rna_XM_060938827_1",
                     "Neoarius_graeffei_rna_XM_060938829_1")) 



pdf("N5.HOG0019032/ks_distribution-Neoarius_graeffei-Chanos_chanos.pdf",width = 8.34,  height = 4.61)
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0019032.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <-  c("Neoarius_graeffei_rna_XM_060939486_1",
              "Neoarius_graeffei_rna_XM_060938827_1",
              "Neoarius_graeffei_rna_XM_060938829_1")

my_seq2 <- c("Chanos_chanos_rna_XM_030792951_1",
             "Chanos_chanos_rna_XM_030788615_1",
             "Chanos_chanos_rna_XM_030788612_1",
             "Chanos_chanos_rna_XM_030788616_1",
             "Chanos_chanos_rna_XM_030788614_1",
             "Chanos_chanos_rna_XM_030788611_1")
...
pdf("N5.HOG0019032/HOG_ks_distribution.pdf",width = 8.34,  height = 4.61)
...
pair_to_fix_onlyscaff <- 
  c("Megalops_atlanticus,Anguilla_anguilla",
    "Ictalurus_punctatus,Silurus_meridionalis",
    "Silurus_meridionalis,Trichomycterus_rosablanca")
...
pair_to_fix_onlyscaff <- 
  c("Clupea_harengus,Chanos_chanos")
...
for(species_pair in pair_to_fix_onlyscaff){
  ...
  my_links.clade1_new <- 
    my_links.clade1 %>% 
    filter(! (species1 == my_sp1 & species2 == my_sp2 & seq_id == "NC_045170_1"))
...
p.clade1 <-  
  ...
  flip_seqs("CM032895_1") %>%
  flip_seqs("CM032883_1") %>%
  flip_seqs("NC_044506_1") %>%
  flip_seqs("NC_060890_1")
...
all_OGG_gray_vec["N5_HOG0019032"] <- "red"
...
all_OGG_gray_vec <- setNames(rep("gray", length(all_OGG)), all_OGG)
all_OGG_gray_vec["N5_HOG0019032"] <- "red"
all_OGG_gray_vec["Merlin-1_DR_1p:ClassII:TIR:Merlin:?"] <- "#FFC107"
...
#### Region alignment -- Neoarius_graeffei versus Chanos_chanos - 1 ---------------------------------

library(dotplot)

Neoarius_region_1 <- seqinr::read.fasta("N5.HOG0019032.Neoarius_graeffei.extended.1.fa.rev", seqtype = "DNA", as.string = TRUE)[[1]]
Chanos_region_1 <- seqinr::read.fasta("N5.HOG0019032.Chanos_chanos.extended.1.dotplot.fa", seqtype = "DNA", as.string = TRUE)[[1]]

dotPlotg(Neoarius_region_1, Chanos_region_1, wsize = 60, wstep = 1, nmatch = 36) +
  geom_rect(aes(xmin=31990, xmax=32700, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=10482 , ymax=11600, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) 

dotplot.p1 <- 
  dotPlotg(Neoarius_region_1, Chanos_region_1, wsize = 60, wstep = 1, nmatch = 36) +
  
  geom_rect(aes(xmin=31960, xmax=31985, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=29486, xmax=29597, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=29011, xmax=29153, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=28472, xmax=28673, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=27972, xmax=28077, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=27713, xmax=27853, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=26188, xmax=26253, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=25841, xmax=25972, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=25498, xmax=25659, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=23872, xmax=23964, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=23580, xmax=23734, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5933,  xmax=6098 , ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5529,  xmax=5695 , ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(xmin=21867,  xmax=22138 , ymin=0, ymax=Inf), fill=NA, color="#E69F00", linewidth=0.2, alpha=0.4) + 
  
  
  geom_rect(aes(ymin=2000 , ymax=2169, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=2406 , ymax=2571, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=5223 , ymax=5377, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=5531 , ymax=5623, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=6619 , ymax=6756, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=6977 , ymax=7076, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=7307 , ymax=7380, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=8256 , ymax=8396, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=8728 , ymax=8833, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=9162 , ymax=9363, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=9908 , ymax=10050, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=10374, ymax=10482, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=16743, ymax=16768, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=20947, ymax=21274, xmin=0, xmax=Inf), fill=NA, color="#E69F00", linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("N. graeffei (NC_083582.1:27011828-27049295)")+
  ylab("C. chanos (NC_044506.1:22821135-22851623)")


ggsave("dotplot-Neoarius_graeffei-Chanos_chanos.1.pdf", 
