...
HGT_candidate_df <- HGT_candidate_df %>% filter(species_1 == "Osmerus_eperlanus" | species_2 == "Osmerus_eperlanus")
...
current_tree <- 
  read.tree("N5.HOG0004480.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0004480.prot.aln"
prot_alignment_trim = "N5.HOG0004480.prot.trimmed.aln"
...
current_tree <- 
  read.tree("N5.HOG0004480/HGT_clade.cds.aln.treefile")
...
DNA_tree <- 
  ...
  xlim(0, 1) +
  geom_treescale(x=0.05, y=5, width=0.1, color='black') 
...
current_tree <- 
  read.tree("missing_annot_plus_annot.aln.treefile")
...
#Add the alignment
prot_alignment = "missing_annot_plus_annot.aln"
...
p2 <- msaplot(p0, prot_alignment, offset=0.5, width=3)

pdf("Missing_genes_phylo.pdf",width = 8.34,  height = 4.61)
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Osmerus_eperlanus_rna_XM_062453720_1"))
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0004480.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <- c("Osmerus_eperlanus_rna_XM_062453720_1")
my_seq2 <- c("Clupea_harengus_rna_XM_031565097_2", "Alosa_sapidissima_rna_XM_042087851_1",
             "Engraulis_encrasicolus_rna_XM_063188421_1")
...
HOG_stats_df_species %>% 
  filter(divergence_time > 150 & divergence_time < 200) %>%
  arrange(ks)
...
pair_to_fix_onlyscaff <- 
  c(
    "Clupea_harengus,Osmerus_eperlanus",
    "Osmerus_eperlanus,Hypomesus_transpacificus")
...
all_OGG_gray_vec["N5_HOG0004480"] <- "red"
...
all_OGG_gray_vec <- setNames(rep("gray", length(all_OGG)), all_OGG)
all_OGG_gray_vec["N5_HOG0004480"] <- "red"
all_OGG_gray_vec["L2-3_EL_1p:ClassI:LINE:Jockey:L2"] <- "#FFC107"
...
#### Region alignment -- Osmerus versus Clupea  ---------------------------------

library(dotplot)

osmerus_region_1 <- seqinr::read.fasta("N5.HOG0004480.Osmerus_eperlanus.extended.1.fa.rev", seqtype = "DNA", as.string = TRUE)[[1]]
clupea_region_1 <- seqinr::read.fasta("N5.HOG0004480.Clupea_harengus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]
...
dotplot.p1 <- 
  dotPlotg(osmerus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 33) + 
  geom_rect(aes(xmin=26371, xmax=22659, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=7739, xmax=7646, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=5062, ymax=5155, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=15421, ymax=19211, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("O. eperlanus (NC_085044.1:6434631-6467776)")+
  ylab("C. harengus (NC_045152.1:4827654-4852619)")


ggsave("dotplot_osmerus_clupea.pdf", dotplot.p1, width=8.34, height=6.34, units="in", scale=2)


dotPlotg(osmerus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 33) + 
  geom_rect(aes(xmin=26371, xmax=29900, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=19000, ymax=23000, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) 
