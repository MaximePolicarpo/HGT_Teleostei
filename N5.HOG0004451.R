...
HGT_candidate_df <- HGT_candidate_df %>% filter(species_1 == "Centroberyx_gerrardi" | species_2 == "Centroberyx_gerrardi")
...
current_tree <- 
  read.tree("N5.HOG0004451.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0004451.prot.aln"
prot_alignment_trim = "N5.HOG0004451.prot.trimmed.aln"
...
current_tree <- 
  read.tree("N5.HOG0004451/HGT_clade.cds.aln.treefile")

outgroup_tips <- c("Anguilla_anguilla_rna_XM_035407034_1",
                   "Anguilla_anguilla_rna_XM_035407033_1",
                   "Anguilla_anguilla_rna_XM_035403412_1",
                   "Conger_conger_rna_XM_061230612_1",
                   "Conger_conger_rna_XM_061232361_1",
                   "Conger_conger_rna_XM_061230610_1",
                   "Conger_conger_rna_XM_061230611_1",
                   "Conger_conger_rna_XM_061233141_1",
                   "Conger_conger_rna_XM_061232904_1",
                   "Conger_conger_rna_XM_061230614_1",
                   "Conger_conger_rna_XM_061230613_1",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116800",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116770",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116790",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116810",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116870",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116890",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116910",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116840",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116860",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116880",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116920",
                   "Synaphobranchus_kaupii_mrna_SKAU_T00116900")


current_tree_rooted <- root(current_tree, outgroup=outgroup_tips)
...
DNA_tree <- 
  ...
  xlim(0, 4) +
  geom_treescale(x=0.02, y=32, width=0.2, color='black') 
...
#### Draw  phylogenies with alignment -- Other Beryx spp   ---------------------------------


current_tree <- 
  read.tree("HOG0004451.clade.Beryx.aln.treefile")
...
toadd <- as.data.frame(cbind("Beryx_splendens.OMOV01022145.1-1931-5459", "Centroberyx_gerrardi", "Beryciformes"))
colnames(toadd) <- colnames(tips_df_species_order)
tips_df_species_order <- rbind(tips_df_species_order, toadd)

#Add the alignment
prot_alignment_trim= "HOG0004451.clade.Beryx.aln.trimmed"
...
pdf("N5.HOG0004451/Manually_annoated_genes.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Centroberyx_gerrardi vs Megalops_cyprinoides   ---------------------------------


current_ks_df <- 
  fread("HGT_stats_Results_per_sp/Centroberyx_gerrardi.cds.stats",
        header=FALSE,
        sep=",")

curr_stats_df <-
  fread("KS_Stats_per_species/Centroberyx_gerrardi.csv",
        header=TRUE,
        sep=",")



species1 <- "Centroberyx_gerrardi"
species2 <- "Megalops_cyprinoides"
...
write.table(current_ks_df %>% dplyr::select(OGG, seq1, seq2),
            "All_pairwise_genes.Megalops.Centroberyx.csv",
            sep=",",
            col.names=FALSE,
            row.names=FALSE,
            quote=FALSE)
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Centroberyx_gerrardi_g23848_t1")) %>%
  filter(seq2 %in% c("Megalops_cyprinoides_rna_XM_036520634_1"))



pdf("N5.HOG0004451/ks_distribution-Centroberyx_gerrardi-Megalops_cyprinoides.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Centroberyx_gerrardi vs Albula_glossodonta   ---------------------------------
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Centroberyx_gerrardi_g23848_t1")) %>%
  filter(seq2 %in% c("Albula_goreensis_AGOR_T00122240",
                     "Albula_glossodonta_mRNA15989",
                     "Albula_glossodonta_mRNA15988",
                     "Albula_glossodonta_mRNA15990",
                     "Albula_glossodonta_mRNA7121"))



pdf("N5.HOG0004451/ks_distribution-Centroberyx_gerrardi-Albula_glossodonta.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Centroberyx_gerrardi vs Aldrovandia_affinis   ---------------------------------
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Centroberyx_gerrardi_g23848_t1")) %>%
  filter(seq2 %in% c("Aldrovandia_affinis_mrna_AAFF_T00102350",
                     "Aldrovandia_affinis_mrna_AAFF_T00314970",
                     "Aldrovandia_affinis_mrna_AAFF_T00314980"))



pdf("N5.HOG0004451/ks_distribution-Centroberyx_gerrardi-Aldrovandia_affinis.pdf",width = 8.34,  height = 4.61)
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0004451.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <-  c("Centroberyx_gerrardi_g23848_t1")

my_seq2 <- c("Megalops_cyprinoides_rna_XM_036520634_1")
my_seq3 <- c("Albula_glossodonta_mRNA15989",
             "Albula_glossodonta_mRNA15988",
             "Albula_glossodonta_mRNA15990",
             "Albula_goreensis_AGOR_T00122240",
             "Albula_glossodonta_mRNA7121")

my_seq4 <- c("Aldrovandia_affinis_mrna_AAFF_T00314980",
             "Aldrovandia_affinis_mrna_AAFF_T00314970",
             "Aldrovandia_affinis_mrna_AAFF_T00102350")
...
pdf("N5.HOG0004451/HOG_ks_distribution.pdf",width = 8.34,  height = 4.61)

HOG_stats_df_species %>%
  ggplot(., aes(x=(divergence_time), y=ks, color=group_hgt)) +
  geom_point() + 
  scale_color_manual(values = c("no_HGT" = "#8A8686", "HGT" = "#D55E00", "HGT_2" = "#009E73", "HGT_3" = "#0072B2")) + 
...
all_OGG_gray_vec["N5_HOG0004451"] <- "red"
...
#### Region alignment -- Megalops_cyprinoides versus Centroberyx_gerrardi  ---------------------------------

library(dotplot)
setwd("N5.HOG0004451")

Centroberyx_region_1 <- seqinr::read.fasta("N5.HOG0004451.Centroberyx_gerrardi.extended.1.fa.rev", seqtype = "DNA", as.string = TRUE)[[1]]
Megalops_region_1 <- seqinr::read.fasta("N5.HOG0004451.Megalops_cyprinoides.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]


  
dotplot.p1 <- 
  dotPlotg(Centroberyx_region_1, Megalops_region_1, wsize = 60, wstep = 1, nmatch = 36)  +
  
  geom_rect(aes(xmin=9014, xmax=9104, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5000, xmax=6429, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=5000 , ymax=6429, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=6819 , ymax=5632, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("C. gerrardi (JAPMTB010005118.1:498411-512515)")+
  ylab("M. cyprinoides (NC_050584.1:69815062-69826893)")


ggsave("dotplot-gerrardi-cyprinoides.1.pdf", 
...
dotPlotg(Centroberyx_region_1, Megalops_region_1, wsize = 60, wstep = 1, nmatch = 36)  +
  ...
  geom_rect(aes(xmin=3800, xmax=5000, ymin=0, ymax=Inf), fill=NA, color="blue", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=4000 , ymax=5000, xmin=0, xmax=Inf), fill=NA, color="blue", linewidth=0.2, alpha=0.4) 
