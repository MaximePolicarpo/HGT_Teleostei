...
current_tree <- 
  read.tree("N5.HOG0030200.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0030200.prot.aln"
prot_alignment_trim = "N5.HOG0030200.prot.trimmed.aln"
...
current_tree <- 
  read.tree("N5.HOG0030200/HGT_clade.cds.aln.treefile")
...
DNA_tree <- 
  ...
  xlim(0, 1) +
  geom_treescale(x=0.02, y=22, width=0.05, color='black') 
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Pangasianodon_hypophthalmus_rna_XM_026945431_3")) %>%
  filter(seq2 %in% c("Cyprinus_carpio_rna_XM_042728143_1"))



pdf("N5.HOG0030200/ks_distribution-Pangasianodon_hypophthalmus-Cyprinus_carpio.pdf",width = 8.34,  height = 4.61)
...
write.table(current_ks_df %>% dplyr::select(OGG, seq1, seq2),
            "All_pairwise_genes.Pangasianodon.Carassius.csv",
            sep=",",
            col.names=FALSE,
            row.names=FALSE,
            quote=FALSE)
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Pangasianodon_hypophthalmus_rna_XM_026945431_3")) %>%
  filter(seq2 %in% c("Carassius_auratus_rna_XM_026200108_1"))



pdf("N5.HOG0030200/ks_distribution-Pangasianodon_hypophthalmus-Carassius_auratus.pdf",width = 8.34,  height = 4.61)
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Pangasius_djambal_PDJAM_T00012360",
                     "Pangasius_djambal_PDJAM_T00103720")) %>%
  filter(seq2 %in% c("Cyprinus_carpio_rna_XM_042728143_1"))


pdf("N5.HOG0030200/ks_distribution-Pangasius_djambal-Cyprinus_carpio.pdf",width = 8.34,  height = 4.61)
...
pdf("N5.HOG0030200/ks_distribution-Pangasius_djambal-Carassius_auratus.pdf",width = 8.34,  height = 4.61)
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0030200.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <- c("Pangasius_djambal_PDJAM_T00012360",
             "Pangasius_djambal_PDJAM_T00103720",
...
pdf("N5.HOG0030200/HOG_ks_distribution.pdf",width = 8.34,  height = 4.61)
...
pair_to_fix_onlyscaff <- 
  c("Silurus_meridionalis,Ictalurus_punctatus")
...
for(species_pair in pair_to_fix_onlyscaff){
  ...
  test <- 
    ...
    filter(seq_id %in% c("NC_060884_1", "NC_060900_1")) %>%
    mutate(start3 = end2) %>%
    mutate(end3 = start2) %>%
  ...
  my_links.clade1_new <- 
    my_links.clade1 %>% 
    filter(! (species1 == my_sp1 & species2 == my_sp2 & seq_id %in% c("NC_060884_1", "NC_060900_1")))
...
pair_to_fix_onlyscaff <- 
  c("Onychostoma_macrolepis,Danionella_translucida")
...
p.clade1 <-  
  ...
  flip_seqs("NC_060884_1") %>%
  flip_seqs("SRMA01025672_1") %>%
  flip_seqs("SRMA01026995_1")
...
all_OGG_gray_vec["N5_HOG0030200"] <- "red"
...
Pangasianodon_region_1 <- seqinr::read.fasta("N5.HOG0030200.Pangasianodon_hypophthalmus.extended.1.fa.rev", seqtype = "DNA", as.string = TRUE)[[1]]
Carassius_region_1 <- seqinr::read.fasta("N5.HOG0030200.Carassius_auratus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]


dotplot.p1 <- 
  dotPlotg(Pangasianodon_region_1, Carassius_region_1, wsize = 60, wstep = 1, nmatch = 36)  +
  
  geom_rect(aes(xmin=9990, xmax=9809, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=9689, xmax=9464, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=9088, xmax=8945, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=8845, xmax=8664, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=8577, xmax=8491, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=8354, xmax=8174, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=7951, xmax=7914, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=7607, xmax=7501, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=7417, xmax=7265, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=7142, xmax=7113, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=6987, xmax=6863, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=10981 , ymax=11105, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=11216 , ymax=11245, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=11368 , ymax=11520, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=11600 , ymax=11706, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=11979 , ymax=12016, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=12408 , ymax=12588, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=12757 , ymax=12843, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=12930 , ymax=13111, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=13219 , ymax=13362, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=13729 , ymax=13957, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=14149 , ymax=14330, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("P. hypophthalmus (NC_069711.1:3821864-3837403)")+
  ylab("C. auratus (NC_039245.1:4797615-4817324)")
...
dotPlotg(Pangasianodon_region_1, Carassius_region_1, wsize = 60, wstep = 1, nmatch = 36)  +
  ...
  geom_rect(aes(xmin=5100, xmax=5900, ymin=0, ymax=Inf), fill=NA, color="blue", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=4900 , ymax=6100, xmin=0, xmax=Inf), fill=NA, color="blue", linewidth=0.2, alpha=0.4)
