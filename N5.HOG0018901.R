...
current_tree <- 
  read.tree("N5.HOG0018901/N5.HOG0018901.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0018901.prot.aln"
prot_alignment_trim = "N5.HOG0018901.prot.trimmed.aln"
...
current_tree <- 
  read.tree("Hypomesus.combined.prot.aln.trimmed.treefile")
...
#Add the alignment
prot_alignment = "Hypomesus.combined.prot.aln.trimmed"
...
p2 <- msaplot(p0, prot_alignment, offset=1.2, width=3)

pdf("Alternative.phylo.pdf",width = 8.34,  height = 4.61)
...
current_tree <- 
  read.tree("~/Horizontal_transfer_project/Candidates_analysis/N5.HOG0018901/close_clade.cds.aln.treefile")
...
DNA_tree <- 
  ...
  xlim(0, 1) +
  geom_treescale(x=0.05, y=20, width=0.1, color='black') 
...
ggsave("~/Horizontal_transfer_project/Candidates_analysis/N5.HOG0018901/DNA_subtree.pdf",
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Hypomesus_transpacificus_rna_XM_047023826_1",
                     "Hypomesus_transpacificus_rna_XM_047025851_1",
                     "Hypomesus_transpacificus_rna_XM_047043933_1",
                     "Hypomesus_transpacificus_rna_XM_047044257_1")) %>%
  filter(seq2 %in% c("Clupea_harengus_rna_XM_042709983_1",
                     "Clupea_harengus_rna_XM_042709984_1",
                     "Clupea_harengus_rna_XM_012826723_2",
                     "Clupea_harengus_rna_XM_042703844_1",
                     "Clupea_harengus_rna_XM_042703846_1"))
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0018901.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <-  c("Hypomesus_transpacificus_rna_XM_047023826_1",
                     "Hypomesus_transpacificus_rna_XM_047025851_1",
                     "Hypomesus_transpacificus_rna_XM_047043933_1",
                     "Hypomesus_transpacificus_rna_XM_047044257_1")

my_seq2 <- c("Clupea_harengus_rna_XM_042709983_1",
                     "Clupea_harengus_rna_XM_042709984_1",
                     "Clupea_harengus_rna_XM_012826723_2",
                     "Clupea_harengus_rna_XM_042703844_1",
                     "Clupea_harengus_rna_XM_042703846_1")
         
my_seq3 <- c("Clupea_harengus_rna_XM_042708796_1",
             "Sardina_pilchardus_rna_XM_062534740_1",
             "Sardina_pilchardus_rna_XM_062535073_1",
             "Sardina_pilchardus_rna_XM_062534582_1",
             "Sardina_pilchardus_rna_XM_062533668_1",
             "Sardina_pilchardus_rna_XM_062527008_1",
             "Sardina_pilchardus_rna_XM_062525940_1",
             "Alosa_sapidissima_rna_XM_042068858_1",
             "Alosa_sapidissima_rna_XM_042083764_1",
             "Alosa_alosa_rna_XM_048233380_1",
             "Engraulis_encrasicolus_rna_XM_063222206_1",
             "Engraulis_encrasicolus_rna_XM_063222559_1",
             "Engraulis_encrasicolus_rna_XM_063222557_1")
...
HOG_stats_df_species %>%
  ...
  geom_point() + 
  scale_color_manual(values = c("no_HGT" = "#8A8686", "HGT" = "#D55E00", "HGT_2" = "#1E88E5")) + 
...
pair_to_fix_onlyscaff <- 
  c("Engraulis_encrasicolus,Alosa_sapidissima")
...
for(species_pair in pair_to_fix_onlyscaff){
  ...
  my_links.clade1_new <- 
    my_links.clade1 %>% 
    filter(! (species1 == my_sp1 & species2 == my_sp2 & seq_id == "NC_085857_1"))
...
all_OGG_gray_vec["N5_HOG0018901"] <- "red"
...
all_OGG_gray_vec["L1-19_SSa_polLINE/L1"] <- "#FFC107"
all_OGG_gray_vec["L1-19_SSa_pol#LINE/L1"] <- "#FFC107"
all_OGG_gray_vec["L1-19_DR_1p:ClassI:LINE:L1:L1"] <- "#FFC107"
all_OGG_gray_vec["L1-19_SSa_1p:ClassI:LINE:L1:L1"] <- "#FFC107"
all_OGG_gray_vec["L1-19_SSa_2p:ClassI:LINE:L1:L1"] <- "#FFC107"
all_OGG_gray_vec["L1-19_XT_1p:ClassI:LINE:L1:L1"] <- "#FFC107"
all_OGG_gray_vec["L1-19_XT_2p:ClassI:LINE:L1:L1"] <- "#FFC107"
...
hypomesus_region_1 <- seqinr::read.fasta("N5.HOG0018901.Hypomesus_transpacificus.extended.1.rev", seqtype = "DNA", as.string = TRUE)[[1]]
clupea_region_1 <- seqinr::read.fasta("N5.HOG0018901.Clupea_harengus.extended.dotplot.fa", seqtype = "DNA", as.string = TRUE)[[1]]


dotplot.p1 <- 
  dotPlotg(hypomesus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 33) +
  
  geom_rect(aes(xmin=6995, xmax=6932, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=6754, xmax=6527, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=6026, xmax=5809, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5725, xmax=5598, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5484, xmax=5337, ymin=0, ymax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 

  geom_rect(aes(xmin=1628, xmax=2027, ymin=0, ymax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=2024, ymax=2171, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=2472, ymax=2599, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=2690, ymax=2907, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=3359, ymax=3767, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=11921, ymax=12068, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=12182, ymax=12309, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=12401, ymax=12618, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=13134, ymax=13475, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=15516, ymax=15600, xmin=0, xmax=Inf, color="#D6B554"), fill=NA, linewidth=0.2, alpha=0.4) + 

  geom_rect(aes(ymin=6016, ymax=6487, xmin=0, xmax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=16097, ymax=16437, xmin=0, xmax=Inf, color="#FFC107"), fill=NA, linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("H. transpacificus (NC_061067.1:9519063-9530253)")+
  ylab("C. harengus (NC_045166.1:15868305-15885905)")
...
dotPlotg(hypomesus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 33) +
  ...
  geom_rect(aes(xmin=500, xmax=1628, ymin=0, ymax=Inf), color="black", fill=NA, linewidth=0.1, alpha=0.1) + 
  geom_rect(aes(xmin=2500, xmax=5300, ymin=0, ymax=Inf), color="black", fill=NA, linewidth=0.1, alpha=0.1) +
  geom_rect(aes(xmin=7000, xmax=7300, ymin=0, ymax=Inf), color="black", fill=NA, linewidth=0.1, alpha=0.1) +
  
  geom_rect(aes(ymin=5000, ymax=6000, xmin=0, xmax=Inf), color="black", fill=NA, linewidth=0.1, alpha=0.1) + 
  geom_rect(aes(ymin=8500, ymax=11900, xmin=0, xmax=Inf), color="black", fill=NA, linewidth=0.1, alpha=0.1) + 
  geom_rect(aes(ymin=3780, ymax=4600, xmin=0, xmax=Inf), color="black", fill=NA, linewidth=0.1, alpha=0.1) +
