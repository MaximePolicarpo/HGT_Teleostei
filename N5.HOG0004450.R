...
current_tree <- 
  read.tree("N5.HOG0004450.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0004450.prot.aln"
prot_alignment_trim = "N5.HOG0004450.prot.trimmed.aln"
...
current_tree <- 
  read.tree("N5.HOG0004450/HGT_clade.cds.aln.treefile")
...
#### Species Ks distribution -- Lates_calcarifer vs Clupea_harengus   ---------------------------------


current_ks_df <- 
  fread("HGT_stats_Results_per_sp/Lates_calcarifer.cds.stats",
        header=FALSE,
        sep=",")

curr_stats_df <-
  fread("KS_Stats_per_species/Lates_calcarifer.csv",
        header=TRUE,
        sep=",")



species1 <- "Lates_calcarifer"
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Lates_calcarifer_rna_XM_018688510_2", "Lates_calcarifer_rna_XM_018686387_2")) %>%
  filter(seq2 %in% c("Clupea_harengus_rna_XM_042704194_1"))



pdf("N5.HOG0004450/ks_distribution-Lates_calcarifer-Clupea_harengus.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Lates_calcarifer vs Sardina_pilchardus   ---------------------------------
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Lates_calcarifer_rna_XM_018688510_2", "Lates_calcarifer_rna_XM_018686387_2")) %>%
  filter(seq2 %in% c("Sardina_pilchardus_rna_XM_062523630_1",
                     "Sardina_pilchardus_rna_XM_062523290_1"))


pdf("N5.HOG0004450/ks_distribution-Lates_calcarifer-Sardina_pilchardus.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Caranx_melampygus vs Sardina_pilchardus   ---------------------------------


current_ks_df <- 
  fread("HGT_stats_Results_per_sp/Caranx_melampygus.cds.stats",
        header=FALSE,
        sep=",")

curr_stats_df <-
  fread("KS_Stats_per_species/Caranx_melampygus.csv",
        header=TRUE,
        sep=",")



species1 <- "Caranx_melampygus"
species2 <- "Sardina_pilchardus"
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Caranx_melampygus_mRNA11584", "Caranx_melampygus_mRNA27263",
                     "Caranx_melampygus_mRNA25461")) %>%
  filter(seq2 %in% c("Sardina_pilchardus_rna_XM_062523630_1",
                     "Sardina_pilchardus_rna_XM_062523290_1"))


pdf("N5.HOG0004450/ks_distribution-Caranx_melampygus-Sardina_pilchardus.pdf",width = 8.34,  height = 4.61)
...
#### Species Ks distribution -- Caranx_melampygus vs Clupea_harengus   ---------------------------------
...
write.table(current_ks_df %>% dplyr::select(OGG, seq1, seq2),
            "All_pairwise_genes.Caranx.Clupea.csv",
            sep=",",
            col.names=FALSE,
            row.names=FALSE,
            quote=FALSE)
...
pdf("N5.HOG0004450/ks_distribution-Caranx_melampygusr-Clupea_harengus.pdf",width = 8.34,  height = 4.61)
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0004450.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <- c("Alosa_alosa_rna_XM_048256059_1",
             "Alosa_sapidissima_rna_XM_042093290_1",
             "Sardina_pilchardus_rna_XM_062523630_1",
             "Sardina_pilchardus_rna_XM_062523290_1",
             "Alosa_sapidissima_rna_XM_042103484_1",
             "Alosa_alosa_rna_XM_048250590_1",
             "Alosa_alosa_rna_XM_048239489_1",
             "Clupea_harengus_rna_XM_042704194_1",
             "Engraulis_encrasicolus_rna_XM_063193637_1",
             "Engraulis_encrasicolus_rna_XM_063193640_1",
             "Engraulis_encrasicolus_rna_XM_063194025_1",
             "Engraulis_encrasicolus_rna_XM_063193635_1")

my_seq2 <-  c("Caranx_melampygus_mRNA25461",
              "Caranx_melampygus_mRNA11584",
              "Caranx_melampygus_mRNA27263",
              "Seriola_lalandi_dorsalis_rna_XM_023399112_1")

my_seq3 <- c("Lates_calcarifer_rna_XM_018686387_2",
...
pdf("N5.HOG0004450/HOG_ks_distribution.pdf",width = 8.34,  height = 4.61)

HOG_stats_df_species %>%
  ggplot(., aes(x=(divergence_time), y=ks, color=group_hgt)) +
  geom_point() + 
  scale_color_manual(values = c("no_HGT" = "#8A8686", "HGT" = "#D55E00", "HGT_2" = "#009E73")) + 
...
all_OGG_gray_vec["N5_HOG0004450"] <- "red"
...
#### Region alignment -- Caranx_melampygus versus Sardina_pilchardus #1  ---------------------------------

library(dotplot)

Caranx_region_1 <- seqinr::read.fasta("N5.HOG0004450.Caranx_melampygus.extended.3.fa", seqtype = "DNA", as.string = TRUE)[[1]]
Sardina_region_1 <- seqinr::read.fasta("N5.HOG0004450.Sardina_pilchardus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]


dotplot.p1 <- 
  dotPlotg(Caranx_region_1, Sardina_region_1, wsize = 60, wstep = 1, nmatch = 36)  +
  
  geom_rect(aes(xmin=3974, xmax=4088, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5041, xmax=6641, ymin=0, ymax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=9310 , ymax=9409, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=9751 , ymax=12242, xmin=0, xmax=Inf), fill=NA, color="indianred1", linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("C. melampygus (JAFELL010002462.1:248780-261895)")+
  ylab("S. pilchardus (NC_085013.1:30433942-30446476)")


ggsave("dotplot-melampygus-pilchardus.1.pdf", 
...
dotPlotg(Caranx_region_1, Sardina_region_1, wsize = 60, wstep = 1, nmatch = 36)  +
  ...
  geom_rect(aes(xmin=6641, xmax=7000, ymin=0, ymax=Inf), fill=NA, color="blue", linewidth=0.2, alpha=0.4)  + 
  geom_rect(aes(ymin=12243 , ymax=12643, xmin=0, xmax=Inf), fill=NA, color="blue", linewidth=0.2, alpha=0.4)  
