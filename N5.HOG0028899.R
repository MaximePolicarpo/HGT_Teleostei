...
current_tree <- 
  read.tree("N5.HOG0028899.prot.aln.treefile")
...
#Add the alignment
prot_alignment = "N5.HOG0028899/N5.HOG0028899.prot.aln"
prot_alignment_trim = "N5.HOG0028899/N5.HOG0028899.prot.trimmed.aln"
...
#### Draw  phylogenies with NONcoding element    ---------------------------------


current_tree <- 
  read.tree("similarity_region_1.combined.aln.trimmed.treefile")
...
#Add the alignment
DNA_alignment = "similarity_region_1.combined.aln.trimmed"
...
p_dna <- 
  ...
  new_scale_color() + 
  theme(legend.position = "none") 
p_dna_1 <- msaplot(p_dna, DNA_alignment, offset=1, width=4)


pdf("N5.HOG0028899/DNA_phylo.pdf",width = 8.34,  height = 4.61)
...
current_tree <- 
  read.tree("N5.HOG0028899/HGT_clade.cds.aln.treefile")
...
DNA_tree <- 
  ...
  xlim(0, 0.5) +
  geom_treescale(x=0.02, y=15, width=0.05, color='black') 
...
current_ks_HGT <-
  current_ks_df %>%
  filter(seq1 %in% c("Hypomesus_transpacificus_rna_XM_047031800_1")) 


pdf("N5.HOG0028899/ks_distribution.hypo_clupea.pdf",width = 8.34,  height = 4.61)
...
HOG_stats_df <- 
  fread("Coding_sequences_alignments/N5.HOG0028899.cds.stats",
        sep=",",
        header=TRUE)
...
my_seq1 <- c("Hypomesus_transpacificus_rna_XM_047031800_1")
my_seq2 <- c("Alosa_sapidissima_rna_XM_042096290_1","Alosa_sapidissima_rna_XM_042096292_1",
             "Alosa_sapidissima_rna_XM_042096285_1","Alosa_sapidissima_rna_XM_042096293_1",
             "Alosa_alosa_rna_XM_048250683_1","Sardina_pilchardus_rna_XM_062551642_1",
             "Engraulis_encrasicolus_rna_XM_063222654_1","Clupea_harengus_rna_XM_031582176_2",
             "Clupea_harengus_rna_XM_042709989_1","Clupea_harengus_rna_XM_031581960_1",
             "Clupea_harengus_rna_XM_031564016_1","Alosa_alosa_rna_XM_048250564_1",
             "Sardina_pilchardus_rna_XM_062550286_1")
...
HOG_stats_df_species %>%
  filter(group_hgt == "HGT")

pdf("N5.HOG0028899/HOG_ks_distribution.pdf",width = 8.34,  height = 4.61)
...
pair_to_fix_onlyscaff <- 
  c(
    "Hypomesus_transpacificus,Osmerus_eperlanus")
...
all_OGG_gray_vec["N5_HOG0028899"] <- "red"
...
hypomesus_region_1 <- seqinr::read.fasta("N5.HOG0028899.Hypomesus_transpacificus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]
clupea_region_1 <- seqinr::read.fasta("N5.HOG0028899.Clupea_harengus.extended.1.fa", seqtype = "DNA", as.string = TRUE)[[1]]


dotplot.p1 <- 
  dotPlotg(hypomesus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 27) +
  geom_rect(aes(xmin=1447, xmax=1567, ymin=0, ymax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=1913, xmax=2087, ymin=0, ymax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=4493, xmax=4595, ymin=0, ymax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(xmin=5002, xmax=5031, ymin=0, ymax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=5000, ymax=5181, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=7239, ymax=7351, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=8729, ymax=8883, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 

  geom_rect(aes(ymin=36272, ymax=36401, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=36768, ymax=36942, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=37779, ymax=37966, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=38719, ymax=38942, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  
  geom_rect(aes(ymin=68560, ymax=68571, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=69005, ymax=69059, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=69314, ymax=69349, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=70273, ymax=70444, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=70862, ymax=70974, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=71780, ymax=71954, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=72296, ymax=72425, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        plot.subtitle=element_text(size=16),
        legend.position="none") +
  xlab("H. transpacificus (NC_061072.1:7354826-7366858)")+
  ylab("C. harengus (NC_045166.1:16191554-16269185)")
...
dotPlotg(hypomesus_region_1, clupea_region_1, wsize = 40, wstep = 1, nmatch = 27) +
  geom_rect(aes(xmin=5050, xmax=5800, ymin=0, ymax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) +
  geom_rect(aes(ymin=10000, ymax=12000, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) +
  geom_rect(aes(ymin=40000, ymax=42000, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) 
...
clupea_region_2 <- seqinr::read.fasta("N5.HOG0028899.Clupea_harengus.extended.2.fa", seqtype = "DNA", as.string = TRUE)[[1]]

dotPlotg(hypomesus_region_1, clupea_region_2, wsize = 40, wstep = 1, nmatch = 27)
...
dotplot.p2 <- 
  ...
  geom_rect(aes(ymin=5158, ymax=5326, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  geom_rect(aes(ymin=6155, ymax=6309, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        legend.position="none") +
  xlab("H. transpacificus (NC_061072.1:7354826-7366858)")+
  ylab("C. harengus (NC_045177.1:8606891-8619407)")
...
dotPlotg(hypomesus_region_1, clupea_region_2, wsize = 40, wstep = 1, nmatch = 27) +
  geom_rect(aes(xmin=5050, xmax=5800, ymin=0, ymax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) +
  geom_rect(aes(ymin=8000, ymax=9200, xmin=0, xmax=Inf), fill=NA, color="#F13D3D", linewidth=0.2, alpha=0.4) 
